...
def x1_to_x2(x1, x2):
    ...
    if isinstance(x1, list) and isinstance(x2, list):
        x1_to_world = x_to_world(x1)  #将 x1 从其坐标系转换到世界坐标系。
        x2_to_world = x_to_world(x2)  #将 x2 从其坐标系转换到世界坐标系。
        world_to_x2 = np.linalg.inv(x2_to_world)  #计算从世界坐标系到 x2 坐标系的变换矩阵，即x2_to_world 的逆矩阵。
        # 通过将世界到 x2 的变换矩阵和 x1 到世界的变换矩阵相乘，得到从 x1 到 x2 的变换矩阵。
