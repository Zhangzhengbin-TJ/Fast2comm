...
def x_to_world(pose):
    ...
    c_y = np.cos(np.radians(yaw))  #将角度转换为弧度
...
def x1_to_x2(x1, x2):
    ...
    """
    x1_to_world = x_to_world(x1)  #The transformation matrix from x-coordinate system to carla world system
    x2_to_world = x_to_world(x2)
    world_to_x2 = np.linalg.inv(x2_to_world)  #矩阵求逆
