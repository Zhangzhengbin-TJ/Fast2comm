...
from opencood.utils.ermvp_transformation_utils import x1_to_x2
...
def mask_boxes_outside_range_numpy(boxes, limit_range, order,
    ...
    if boxes.shape[1] == 7:
        new_boxes = boxes_to_corners_3d(new_boxes, order) #根据中心点位置（x,y,z）/框尺寸h,w,l/朝向角计算框体的8个角点。
...
def project_world_objects(object_dict,
    ...
    order : str
        'lwh' or 'hwl'
    """
    for object_id, object_content in object_dict.items():  #遍历vehicle编号及其信息
        ...
        # project the 8 corners to world coordinate  #将八个角点坐标投影到世界坐标
        bbx_lidar = np.dot(object2lidar, bbx).T
        bbx_lidar = np.expand_dims(bbx_lidar[:, :3], 0)  #(4,8)->(1,8,3)
        bbx_lidar = corner_to_center(bbx_lidar, order=order)  #Convert 8 corners to x, y, z, dx, dy, dz, yaw.
        bbx_lidar = mask_boxes_outside_range_numpy(bbx_lidar,
                                                   lidar_range,
                                                   order)

        if bbx_lidar.shape[0] > 0:
            output_dict.update({object_id: bbx_lidar})  #将bbx_lidar信息放入output_dict中
...
def project_points_by_matrix_torch(points, transformation_matrix):
    ...
    points_homogeneous = F.pad(points, (0, 1), mode="constant", value=1)
    # (N, 4)
    projected_points = torch.einsum("ik, jk->ij", points_homogeneous,  #矩阵乘法
                                    transformation_matrix)
