...
from cumm.dtypes import float64
...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_anchor_box(self):
        ...
        H = self.params['anchor_args']['H']  #这里两个参数应该是处理后得到

        l = self.params['anchor_args']['l']
        w = self.params['anchor_args']['w']
        h = self.params['anchor_args']['h']
        r = self.params['anchor_args']['r']   #这里的参数从配置文件中得到
        ...
        if 'feature_stride' in self.params['anchor_args']:
            ...
            feature_stride = 2
        #x:在140到-140的范围内产生704/4个数值，每个数值等分280，y:在38到-38的范围内产生192/4个数值，每个数值等分76
        x = np.linspace(xrange[0] + vw, xrange[1] - vw, W // feature_stride)  #创建数值序列
        y = np.linspace(yrange[0] + vh, yrange[1] - vh, H // feature_stride)

        cx, cy = np.meshgrid(x, y)  #生成y行x列的数组，(48,176)
        cx = np.tile(cx[..., np.newaxis], self.anchor_num)  #复制cx,(48,176,2)
        cy = np.tile(cy[..., np.newaxis], self.anchor_num)  #(48,176,2)
        cz = np.ones_like(cx) * -1.0

        w = np.ones_like(cx) * w  #用1填充的跟输入形状和类型一致的数组，再乘w
        l = np.ones_like(cx) * l
        h = np.ones_like(cx) * h
        ...
        return anchors
    def transfor2featuremap(self, gt_standup_2d):
        '''
        transfor 2d_standup_box_gt from world coordinates to feature size and location
        input:(num_cav, 4), 2d_standup_box_gt under world coordinates
        output:(num_cav, 6), transformed 2d_standup_box_gt,(x1,y1,x2,y2,width,length)
        '''
        gt_standup_2d_transformedfeaturemap = []
        for gt in gt_standup_2d:
            x1, y1, x2, y2 = gt
            x1 = ((x1 + 140)/ (280 / 176))*2
            x1 = math.floor(x1)
            # x1 = math.ceil(x1)
            ...
            gt_standup_2d_transformedfeaturemap.append([x1, y1, x2, y2, width, height])
        return gt_standup_2d_transformedfeaturemap
    ...
    def generate_label(self, **kwargs):
        ...
        assert self.params['order'] == 'hwl', 'Currently Voxel only support' \
                                              'hwl bbx order.'
        # (max_num, 7)
        gt_box_center = kwargs['gt_box_center']  #(100,7)
        # (H, W, anchor_num, 7)
        anchors = kwargs['anchors']  #(48,176,2,7)先验边界框，特征图上的每个点生成多个边界框
        # (max_num)
        masks = kwargs['mask']  #(100)有效目标的布尔标识

        # (H, W)
        feature_map_shape = anchors.shape[:2]  #(48,176)

        # (H*W*anchor_num, 7)
        anchors = anchors.reshape(-1, 7)  #(48x176x2=16896, 7)，因为特征图大小为48x176，在每个点都生成了两个anchor
        # normalization factor, (H * W * anchor_num)
        anchors_d = np.sqrt(anchors[:, 4] ** 2 + anchors[:, 5] ** 2)  #(16896),[4.215,4.215,...]
        # 计算 anchors 的宽高平方和的平方根，用于归一化位置偏差

        # (H, W, 2)
        pos_equal_one = np.zeros((*feature_map_shape, self.anchor_num))  #(48,176,2)
        neg_equal_one = np.zeros((*feature_map_shape, self.anchor_num))  #(48,176,2)
        # (H, W, self.anchor_num * 7)
        targets = np.zeros((*feature_map_shape, self.anchor_num * 7))  #(48,176,2*7)

        # (n, 7)
        gt_box_center_valid = gt_box_center[masks == 1]  #n表示一个batch下的车辆数，筛选有效的真实目标
        # (n, 8, 3)
        gt_box_corner_valid = \
            box_utils.boxes_to_corners_3d(gt_box_center_valid,
                                          self.params['order'])  #将bbox参数转换为八个角点坐标
        # (H*W*anchor_num, 8, 3)
        anchors_corner = \
            box_utils.boxes_to_corners_3d(anchors,
                                          order=self.params['order'])  #(16896,8,3)
        # (H*W*anchor_num, 4)
        anchors_standup_2d = \
            box_utils.corner2d_to_standup_box(anchors_corner)  #找出最大的x、y与最小的x、y坐标值
        # (n, 4)
        gt_standup_2d = \
            box_utils.corner2d_to_standup_box(gt_box_corner_valid)
        gt_standup_2d_transfor2featuremap = self.transfor2featuremap(gt_standup_2d)
        # gt_standup_2d_transformedfeaturemap = np.array(gt_standup_2d_transfor2featuremap,dtype=np.float64)

        # (H*W*anchor_num, n)
        iou = bbox_overlaps(
            np.ascontiguousarray(anchors_standup_2d).astype(np.float32),  #ascontiguousarray将内存不连续变为内存连续
            np.ascontiguousarray(gt_standup_2d).astype(np.float32),
        )  #(16896, n)

        # the anchor boxes has the largest iou across
        # shape: (n)
        id_highest = np.argmax(iou.T, axis=1)  #取iou最大值的索引
        # [0, 1, 2, ..., n-1]
        ...
        id_pos, id_pos_gt = \
            np.where(iou >
                     self.params['target_args']['pos_threshold']) #iou大于阈值的索引
        ...
        index_x, index_y, index_z = np.unravel_index(
            id_pos, (*feature_map_shape, self.anchor_num))  #找id_pos位于数组(48,176,2)的索引，返回三维的数组
        pos_equal_one[index_x, index_y, index_z] = 1

        # calculate the targets
        #对正样本，计算真实目标与 anchor 的位置偏差和尺度比，并归一化存储到 targets 中
        ...
        targets[index_x, index_y, np.array(index_z) * 7 + 6] = (
                gt_box_center[id_pos_gt, 6] - anchors[id_pos, 6])  #(48,176,14)
        ...
        label_dict = {'pos_equal_one': pos_equal_one,
                      ...
                      'targets': targets,
                      'gt_standup_2d_transfor2featuremap': gt_standup_2d_transfor2featuremap
                      }
    ...
    def collate_batch(label_batch_list):
        ...
        neg_equal_one = []
        targets = []
        gt_standup_2d_transfor2featuremap = []
        ...
        for i in range(len(label_batch_list)):
            ...
            gt_standup_2d_transfor2featuremap.append(label_batch_list[i]['gt_standup_2d_transfor2featuremap'])
        ...
        # gt_standup_2d_transfor2featuremap = \
        #     torch.from_numpy(np.array(gt_standup_2d_transfor2featuremap))
