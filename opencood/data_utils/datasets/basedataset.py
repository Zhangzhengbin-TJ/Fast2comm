...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.len_record = []  #len（len_record）长度是场景个数， 就是train文件夹里面的文件个数
                                # 列表里面的数值是每个车采样帧数（时间戳个数）
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            cav_list = sorted([x for x in os.listdir(scenario_folder)  #cav_list是文件夹里面的文件个数，如641,650,659
                               if os.path.isdir(
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                yaml_files = \
                    sorted([os.path.join(cav_path, x)
                            for x in os.listdir(cav_path) if  #os.listdir列出文件路径
                            x.endswith('.yaml') and 'additional' not in x])  #endswith#判断字符串是否以指定字符或子字符串结尾\
                #查找'/home/zzbhhh/dataset/OpenCOOD/train/2021_08_16_22_26_54/641'中以yaml结尾的文件
                timestamps = self.extract_timestamps(yaml_files)  #000069,000071， ...

                for timestamp in timestamps:  #加载每一个时间戳的yaml、pcd和image信息
    ...
    def __len__(self):
        return self.len_record[-1]  #返回len_record的长度(6374)
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for i, ele in enumerate(self.len_record):
            if idx < ele:  #idx为随机选择的数
                scenario_index = i  #满足时间戳大于idx的场景编号
                break
        scenario_database = self.scenario_database[scenario_index]  #选择时间戳大于idx的场景

        # check the timestamp index
        timestamp_index = idx if scenario_index == 0 else \
            idx - self.len_record[scenario_index - 1]  #这里没看懂，为什么要这样？
        # retrieve the corresponding timestamp key
        timestamp_key = self.return_timestamp_key(scenario_database,
                                                  timestamp_index)  #获取场景scenario_index中第timestamp_index的时间戳
        # calculate distance to ego for each cav
        ego_cav_content = \
            self.calc_dist_to_ego(scenario_database, timestamp_key)  #计算ego车辆与其他协同车辆的距离

        data = OrderedDict()
        # load files for all CAVs
        for cav_id, cav_content in scenario_database.items():  #cav_id为车辆id, cav_content为时间戳
            data[cav_id] = OrderedDict()
            data[cav_id]['ego'] = cav_content['ego']

            # calculate delay for this vehicle
            timestamp_delay = \
                self.time_delay_calculation(cav_content['ego'])  #计算延时

            if timestamp_index - timestamp_delay <= 0:
                timestamp_delay = timestamp_index
            timestamp_index_delay = max(0, timestamp_index - timestamp_delay)  #计算延迟时间戳的索引
            timestamp_key_delay = self.return_timestamp_key(scenario_database,  #得到延迟时间戳
                                                            timestamp_index_delay)
            # add time delay to vehicle parameters
            data[cav_id]['time_delay'] = timestamp_delay
            # load the corresponding data into the dictionary
            data[cav_id]['params'] = self.reform_param(cav_content,  #当前车辆相关的图像、点云信息
                                                       ego_cav_content,  #ego车辆的相关信息（增加车辆到ego车辆的距离信息）
                                                       timestamp_key,  #时间戳
                                                       timestamp_key_delay,  #带延迟的时间戳
                                                       cur_ego_pose_flag)
            data[cav_id]['lidar_np'] = \
                pcd_utils.pcd_to_np(cav_content[timestamp_key_delay]['lidar'])  #带延迟的时间戳的点云数据
        return data
    ...
    @staticmethod
    def return_timestamp_key(scenario_database, timestamp_index):  #返回场景中索引为timestamp_index的时间戳
        """
    ...
    def add_loc_noise(self, pose, xyz_std, ryp_std):
        ...
        noise_pose = [pose[0] + xyz_noise[0], #改变了pose噪声
                      pose[1] + xyz_noise[1],
                      pose[2] + xyz_noise[2],
                      pose[3],
                      pose[4] + ryp_std[1],
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        if cur_ego_pose_flag:
            transformation_matrix = x1_to_x2(delay_cav_lidar_pose,  #转移矩阵
                                             cur_ego_lidar_pose)
            spatial_correction_matrix = np.eye(4)  #空间校正矩阵
        else:
        ...
        gt_transformation_matrix = x1_to_x2(cur_cav_lidar_pose,
                                            cur_ego_lidar_pose)  #gt_transformation_matrix应该和transformation_matrix是相等的
