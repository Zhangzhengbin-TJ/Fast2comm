...
class AttentionFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.att(x, x, x)
        x_fuse = x.permute(1, 2, 0).view(cav_num, C, H, W)[0]  # C, W, H before
        psm_fuse = x.permute(1, 2, 0).view(cav_num, C, H, W)  #融合后的所有特征
        return x_fuse, psm_fuse
        # return x_fuse
class PointPillarFast2comm(nn.Module):
    def __init__(self, args):
        super(PointPillarFast2comm, self).__init__()
        ...
        # self.fusion_net = Fast2commMultiHead(args['Fast2comm_fusion'])
        self.fusion_net = Fast2comm(args['Fast2comm_fusion'])
    ...
    def forward(self, data_dict):
        ...
        if self.multi_scale:
            ...
            fused_feature, communication_rates = self.fusion_net(batch_dict['spatial_features'],
                                                                 ...
                                                                 pairwise_t_matrix,
                                                                 self.backbone)
            # fused_feature = self.fused_feature_proj(fused_feature)
            if self.shrink_flag:
