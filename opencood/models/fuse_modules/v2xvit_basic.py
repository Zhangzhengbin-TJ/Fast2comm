...
class RelTemporalEncoding(nn.Module):
    ...
    def __init__(self, n_hid, RTE_ratio, max_len=100, dropout=0.2):
        ...
        emb.weight.data[:, 1::2] = torch.cos(position * div_term) / math.sqrt(
            n_hid)
        emb.requires_grad = False  ###论文中不是说可以学习的吗？
...
class RTE(nn.Module):
    def __init__(self, dim, RTE_ratio=2):
        ...
        self.emb = RelTemporalEncoding(dim, RTE_ratio=self.RTE_ratio)  #dim=256
...
class V2XTEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.layers = nn.ModuleList([])
        if self.use_RTE:
            self.rte = RTE(cav_att_config['dim'], self.RTE_ratio) #cav_att_config['dim']：256
    ...
    def forward(self, x, mask, spatial_correction_matrix):
        '''
        x:(b,max_cav,h,w,dim+3)
        '''
        # transform the features to the current timestamp
        # velocity, time_delay, infra
        ### (B,L,H,W,3)，取最后三个，就是编码
        prior_encoding = x[..., -3:]
        ### (B,L,H,W,C)，除编码外的特征
        x = x[..., :-3]
        ###RelTemporalEncoding
        if self.use_RTE:  #就是论文中的delay-aware positional encoding
            # dt: (B,L)
            dt = prior_encoding[:, :, 0, 0, 1].to(torch.int)
            x = self.rte(x, dt)  ###x:(b,5,48,176,256)
        x = self.sttf(x, mask, spatial_correction_matrix) ###(b,5,48,176,256)
        ###An ROI mask is also calculated to prevent the
        # network from paying attention to the padded zeros caused by the spatial warp.
        com_mask = mask.unsqueeze(1).unsqueeze(2).unsqueeze( ###(b,h,w,1,cav)
