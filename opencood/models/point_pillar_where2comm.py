...
class PointPillarWhere2comm(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # N, C, H', W': [N, 384, 96, 352]
        spatial_features_2d = batch_dict['spatial_features_2d']
        # Down-sample feature to reduce memory
        if self.shrink_flag:
            spatial_features_2d = self.shrink_conv(spatial_features_2d) #(B,256,48,176)

        psm_single = self.cls_head(spatial_features_2d)  #为什么只有分类结果？  (N,2,48,176)
        # Compressor
        if self.compression:
