...
class _BaseConvNormActLayer(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_norm:
            norm_type = f"batch_norm_{self.ndim}d"
            norm_layer = get_normalization_layer(num_features=out_channels, norm_type=norm_type, num_groups=groups,
                                             momentum=0.1)
        ...
        #     )
        if use_act:
            act_layer = build_activation_layer(act_type="swish", inplace=True, negative_slope=True,
